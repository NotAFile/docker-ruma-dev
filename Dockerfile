...
RUN apt-get update && \
    ...
    rm rustup-init && \
    curl -LOs https://github.com/jedisct1/libsodium/releases/download/1.0.10/libsodium-1.0.10.tar.gz && \
    tar -zxvf libsodium-1.0.10.tar.gz && \
    (cd libsodium-1.0.10 && ./configure && make && make check && make install) && \
